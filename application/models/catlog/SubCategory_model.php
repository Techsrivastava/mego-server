...
class SubCategory_model extends CI_Model
{
	...
	*/
	public function getSubCategory($postData = array()){
		$sql = "select * from pr_sub_category";		
	...
	*/
	public function editModel($data,$id){
		//echo $id; exit;
		$tbl=DB_PREFIX.'sub_category';
		$sql = "update ".$tbl." set SUB_CATEGORY_UPDATED_AT='".$data["SUB_CATEGORY_UPDATED_AT"]."'";
		if(isset($data["SUB_CATEGORY_NAME"]))
		{
		$sql.=",SUB_CATEGORY_NAME='".$data["SUB_CATEGORY_NAME"]."'";	
		}
		if(isset($data["SUB_CATEGORY_STATUS"]))
		{
		$sql.=",SUB_CATEGORY_STATUS='".$data["SUB_CATEGORY_STATUS"]."'";	
		}
		if(isset($data["SUB_CATEGORY_DESC"]))
		{
		$sql.=",SUB_CATEGORY_DESC='".$data["SUB_CATEGORY_DESC"]."'";	
		}
		if(isset($data["SUB_CATEGORY_IMAGE"]))
		{
		$sql.=",SUB_CATEGORY_IMAGE='".$data["SUB_CATEGORY_IMAGE"]."'";	
		}
		if(isset($data["SUB_CATEGORY_ICON"]))
		{
		$sql.=",SUB_CATEGORY_ICON='".$data["SUB_CATEGORY_ICON"]."'";	
		}
		if(isset($data["SUB_CATEGORY_SORT_ORDER"]))
		{
		$sql.=",SUB_CATEGORY_SORT_ORDER='".$data["SUB_CATEGORY_SORT_ORDER"]."'";	
		}
	...
	*/
	public function deleteModel($id){	
		$tbl=DB_PREFIX.'sub_category';
		$sql = "delete from ".$tbl." where SUB_CATEGORY_ID ='".$id."'";
		if($this->db->query($sql))
		{	
	...
	public function getData($filter = array()) {
        ...
        if (isset($filter['SUB_CATEGORY_ID'])) {
            $this->db->where('sc.SUB_CATEGORY_ID', $filter['SUB_CATEGORY_ID']);
        }      

        if (isset($filter['offset']) && isset($filter['limit'])) {
            $this->db->limit($filter['limit'], $filter['offset']);
        }
        $this->_subcategory =DB_PREFIX.'sub_category as sc';
    ...
    public function getsubCategoryByCat($cat_id)
    {
		$sql = "select * from ".DB_PREFIX."sub_category where CATEGORY_ID='".$cat_id."'";		
		if($query = $this->db->query($sql))
		{
			return $query->result();
		}
